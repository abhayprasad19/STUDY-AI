...
const NoteCard = ({ note, onDelete, onChat, onSummarize, onQuiz }) => {
    ...
    >
      ...
      {note.tags?.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {note.tags.slice(0, 3).map((tag, i) => (
            <span
              key={i}
              className="text-xs px-1.5 py-0.5 rounded bg-white/5 text-slate-500"
            >
              #{tag}
            </span>
          ))}
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-white/8">
        ...
        <button
          onClick={() => onQuiz(note)}
          disabled={!note.isProcessed}
          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-medium bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          <MdQuiz size={13} /> Quiz
        </button>
        <button
